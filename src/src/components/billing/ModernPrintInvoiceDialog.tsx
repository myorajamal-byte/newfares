...
export default function ModernPrintInvoiceDialog({
  ...
  const [invoiceType, setInvoiceType] = useState<'طباعة' | 'تركيب' | 'طباعة وتركيب'>('طباعة');
  ...
  const handlePrint = () => {
    ...
    const printInvoice = async () => {
      try {
        ...
        const fileName = `فاتورة ${invoiceType}_${customerNameForFile}_عقود_${contractsList}_${dateFormatted}`;
            ...
            <div class="invoice-container">
              <div class="header">
                ...
                <div class="invoice-info">
                  <div class="invoice-title">فاتورة ${invoiceType}</div>
              ...
              <div class="customer-info">
                ...
                <div class="customer-details">
                  ...
                  <strong>العقود المرتبطة:</strong> ${sanitizedSelectedContracts.length ? sanitizedSelectedContracts.join(', ') : 'غير محدد'}
                </div>
              </div>
                  ...
                  ${displayItems.map((item, index) => {
                    ...
                    return `
                      <tr class="${isEmpty ? 'empty-row' : ''}">
                        ...
                        <td>${isEmpty ? '' : (typeof item.width === 'number' && typeof item.height === 'number' ? `<span class="num">${item.width}</span> × <span class="num">${item.height}</span>` : '')}</td>
                        <td>${isEmpty ? '' : (typeof item.area === 'number' ? `<span class="num">${item.area.toFixed(2)}</span> م²` : item.area)}</td>
    ...
    <div className="bg-background text-foreground p-6 rounded-lg border border-border shadow-card w-full" dir="rtl">
      ...
      <div className="flex justify-between items-start mb-6 pb-4 border-b-2 border-primary">
        ...
        <div className="text-left" dir="ltr">
          <h1 className="text-3xl font-bold text-primary mb-3">فاتورة ${invoiceType}</h1>
          <div className="text-sm text-muted-foreground">
            رقم الفاتورة: {invoiceNumber}<br />
      ...
      <div className="expenses-preview-item mb-6 p-4 border-r-4 border-primary">
        ...
        <div className="text-sm space-y-1">
          ...
          <div><strong>العقود المرتبطة:</strong> {sanitizedSelectedContracts.length ? sanitizedSelectedContracts.join(', ') : 'غير محدد'}</div>
            ...
            <table className="w-full border-collapse border border-border text-sm">
              ...
              <tbody>
                {localPrintItems.map((item, index) => (
                    ...
                    <td className="border border-border p-3 text-center num">{item.width} × {item.height}</td>
                    <td className="border border-border p-3 text-center num">{item.area.toFixed(2)} م²</td>
                ...
                <Card className="expenses-preview-card">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <Select value={invoiceType} onValueChange={(value: 'طباعة' | 'تركيب' | 'طباعة وتركيب') => setInvoiceType(value)}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="طباعة وتركيب">طباعة وتركيب</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
