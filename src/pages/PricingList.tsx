...
export default function PricingList() {
  ...
  const saveNewSize = async () => {
    ...
    try {
      ...
      // التحقق من وجود المقاس في قائمة الأسعار للمستوى المحدد
      const existingPricing = pricingData.filter(p => 
        p.size === sz && p.billboard_level === selectedLevel
      );

      if (existingPricing.length > 0) {
        toast.error('هذا المقاس موجود بالفعل في قائمة الأسعار لهذا المستوى');
        return;
      }

      // الحصول على جميع الفئات للمستوى المحدد
      ...
      // إنشاء سجلات أسعار للمقاس الجديد لجميع الفئات
  ...
  const availableSizesForLevel = useMemo(() => {
    ...
    // جميع المقاسات الموجودة في النظام (من جدول الأسعار)
